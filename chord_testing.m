...
for j = 1:length(freqBins)-1
    ...
    if ~isempty(bin_magnitudes)
        ...
        if maxMag >= globalMax / 20
            ...
            temp_mag = [temp_mag, maxMag / 1];   % Normalized maximum magnitude in this bin
        end
    end
end
...
matrixName = sprintf('freq_mag_%s', pitch); 
freq_mag_funda = eval(matrixName); % valuate the matrix name
...
for i = 1:length(frequencies_funda)
    ...
    if any(freq_diff <= 3)
        ...
        matching_freq = [matching_freq, frequencies_funda(i)];
        matching_mag_funda = [matching_mag_funda, freq_mag_funda(2, i)];  % Magnitude from freq_mag_funda
        matching_mag_chord = [matching_mag_chord, freq_magnitude(2, matchIdx)];  % Magnitude from freq_magnitude
        
        % Subtract the magnitudes and store the result
        correspondingMagnitude = matching_mag_chord(end); % Ensure this value is set correctly
        subtracted_mag = correspondingMagnitude - lowestFreqMagnitude*matching_mag_funda(end);
